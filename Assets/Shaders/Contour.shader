    ...
    SubShader 
	{
		...
		Pass 
		{
            ...
            fixed4 frag(v2f_img i) : SV_Target 
			{
				...
				finalColor = float3(0.5,0.5,0.5) - contour;

				//finalColor = OffsetHSV(color, float3(0,0,-contour.x * _ContourStrength)); //color - contour;
				//finalColor = color - contour;
				//else if(_ContourOptions == 1) finalColor = color;
				//else if(_ContourOptions == 2) finalColor = float3(1,1,1) - contour;

				//if(id3 < 0 && abs(id3-id5) == 1) finalColor = float4(1,1,1,1);
				//if(id1 < 0 && abs(id1-id7) == 1) finalColor = float4(1,1,1,1);

				if(id3 == id5 && id1 == id7) finalColor = float4(1,1,1,1);
